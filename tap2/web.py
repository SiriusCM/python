...
def getTapData():
    ...
    tapData = select(engine,
                     'SELECT ltrim(role_id) AS role_id,role_name,`level`,awakeLevel AS `超我等级`,portrait AS `头像`,portraitFrame AS `头像框`,thiefNum AS `怪盗数量`, personaNum AS `人格面具`, achievementNum AS `成就`,`rank` AS `心之海段位`,unionBossScore AS `公会Boss总分`,questName AS `玩家主线进度` FROM game_tap_human_0 WHERE role_id = ' + userId)
    if len(tapData) == 0:
        engine = engine1
        tapData = select(engine,
